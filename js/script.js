...
data.forEach(function (people) {
  ...
  /*-2-*/
  /*get names and length friends*/
  _names = _.concat(_names, {
    'name': people.name,
    'friends': people.friends.length
  });

  /*-3-*/
  /*get all friend all people*/
  _.forEach(people.friends, function (friend) {
    friends = _.concat(friends, friend.name);
  });
  /*delete repeatelements*/
  friends = _.uniq(friends);

});

/*2*/
/*sort names depending on friends*/
_.forEach(_.sortBy(_names, 'friends'), function (people) {
    names = _.concat(names, people.name);
});
